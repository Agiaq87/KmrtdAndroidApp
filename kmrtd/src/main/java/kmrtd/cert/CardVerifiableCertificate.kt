...
import kmrtd.cert.CVCAuthorizationTemplate.Companion.from
import kmrtd.cert.CVCAuthorizationTemplate.Companion.fromPermission
import kmrtd.cert.CVCAuthorizationTemplate.Companion.fromRole
...
data class CardVerifiableCertificate(
    ...
    val cvCertificate: CVCertificate,
    @Transient
    val rsaKeyFactory: KeyFactory = KeyFactory.getInstance("RSA")
) :
    Certificate("CVC") {

    /*private val cvCertificate: CVCertificate*/
   ...
   */
    ...
    /*constructor(
        ...
        try {
            val authorityRef = CAReferenceField(
                authorityReference.country.toAlpha2Code(),
                authorityReference.mnemonic,
                authorityReference.seqNumber
            )
            val holderRef = HolderReferenceField(
                holderReference.country.toAlpha2Code(),
                holderReference.mnemonic,
                holderReference.seqNumber
            )
            val authRole = fromRole(role)
            val accessRight = fromPermission(permission)
    ...
    val sigAlgName: String?
        get() = runCatching {
            AlgorithmUtil.getAlgorithmName(cvCertificate.certificateBody.publicKey.objectIdentifier)
        }.getOrNull()
    ...
    val sigAlgOID: String?
         ...
         */
        get() = runCatching {
            cvCertificate.certificateBody.publicKey.objectIdentifier.asText
        }.getOrNull()
    ...
    override fun getEncoded(): ByteArray? = try {
     ...
     */
    override fun toString(): String = cvCertificate.toString()
    ...
    companion object {
        private const val serialVersionUID = -3585440601605666288L
