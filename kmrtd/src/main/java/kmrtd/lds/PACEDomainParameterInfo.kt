...
class PACEDomainParameterInfo @JvmOverloads constructor(
   ...
   */
    ...
    val parameters: AlgorithmParameterSpec
         ...
         */
        get() {
            check(ID_DH_PUBLIC_NUMBER != oid) { "DH PACEDomainParameterInfo not yet implemented" }
            if (ID_EC_PUBLIC_KEY == oid) {
                return toECParameterSpec(domainParameter)
            } else {
                throw IllegalStateException("Unsupported PACEDomainParameterInfo type " + oid)
            }
        }
    ...
    override fun toString(): String {
        return StringBuilder()
            ...
            .append("algorithm: ")
            .append(domainParameter.getAlgorithm().getId()) // e.g. ID_EC_PUBLIC_KEY
            .append(", ")
            .append("parameters: ")
            .append(domainParameter.getParameters()) // e.g. ASN1 sequence of length 6
    ...
    override fun hashCode(): Int {
        return (111111111
                + 7 * oid.hashCode() + 5 * domainParameter.hashCode() + 3 * (if (parameterId == null) 333 else parameterId.hashCode()))
    }
    ...
    companion object {
        ...
        fun checkRequiredIdentifier(oid: String?): Boolean {
            return SecurityInfo.Companion.ID_PACE_DH_GM == oid
                    || SecurityInfo.Companion.ID_PACE_ECDH_GM == oid
                    || SecurityInfo.Companion.ID_PACE_DH_IM == oid
                    || SecurityInfo.Companion.ID_PACE_ECDH_IM == oid
                    || SecurityInfo.Companion.ID_PACE_ECDH_CAM == oid
        }
                ...
                if (x962params.isNamedCurve()) {
                    ...
                    val bcECNamedCurveParams =
                        ECNamedCurveParameterSpec(
                            X962NamedCurves.getName(x96ParamsOID),
                            x9ECParams.getCurve(),
                            x9ECParams.getG(),
                            x9ECParams.getN(),
                            x9ECParams.getH(),
     ...
     */
         ...
         */
        private fun toAlgorithmIdentifier(
            protocolOID: String?,
            parameters: ASN1Encodable?
        ): AlgorithmIdentifier {
            if (SecurityInfo.Companion.ID_PACE_DH_GM == protocolOID
                || SecurityInfo.Companion.ID_PACE_DH_IM == protocolOID
            ) {
                return AlgorithmIdentifier(ASN1ObjectIdentifier(ID_DH_PUBLIC_NUMBER), parameters)
            } else if (SecurityInfo.Companion.ID_PACE_ECDH_GM == protocolOID
                || SecurityInfo.Companion.ID_PACE_ECDH_IM == protocolOID
                || SecurityInfo.Companion.ID_PACE_ECDH_CAM == protocolOID
            ) {
        ...
        private fun toProtocolOIDString(oid: String?): String? {
            if (SecurityInfo.Companion.ID_PACE_DH_GM == oid) {
                return "id-PACE-DH-GM"
            }
            if (SecurityInfo.Companion.ID_PACE_ECDH_GM == oid) {
                return "id-PACE-ECDH-GM"
            }
            if (SecurityInfo.Companion.ID_PACE_DH_IM == oid) {
                return "id-PACE-DH-IM"
            }
            if (SecurityInfo.Companion.ID_PACE_ECDH_IM == oid) {
                return "id-PACE-ECDH-IM"
            }
            if (SecurityInfo.Companion.ID_PACE_ECDH_CAM == oid) {
