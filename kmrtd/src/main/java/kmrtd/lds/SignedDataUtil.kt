...
object SignedDataUtil {
    ...
    fun getDigestEncryptionAlgorithm(signedData: SignedData): String? {
        try {
            val signerInfo = getSignerInfo(signedData)
            val digestEncryptionAlgorithmOID =
                signerInfo.getDigestEncryptionAlgorithm().getAlgorithm().getId()
    ...
    fun createSignerInfo(
        ...
        val encryptedDigestObject: ASN1OctetString =
            DEROctetString(encryptedDigest) // this is the signature
    ...
    fun createAuthenticatedAttributes(
        ...
        val messageDigestAttribute = Attribute(
            ASN1ObjectIdentifier(RFC_3369_MESSAGE_DIGEST_OID),
            createSingletonSet(digestedContent)
        )
        val result = arrayOf<ASN1Object?>(
            contentTypeAttribute.toASN1Primitive(),
    ...
    fun signData(
        ...
        try {
            val dataToBeSigned = createAuthenticatedAttributes(
                digestAlgorithm,
                contentTypeOID,
                contentInfo
            ).getEncoded(
    ...
    private fun checkEContent(
        ...
        for (attribute in attributes) {
            ...
            val storedDigestedContent =
                (attrValuesSet.getObjectAt(0) as ASN1OctetString).getOctets()
    ...
    private fun getAttributes(signedAttributesSet: ASN1Set): MutableList<Attribute> {
        val attributeObjects: MutableList<ASN1Sequence?> =
            Collections.list<Any?>(signedAttributesSet.getObjects())
    ...
    private fun getDigestEncryptionAlgorithmObject(
        ...
        val digestEncryptionAlgOID =
            ASN1ObjectIdentifier(lookupOIDByMnemonic(digestEncryptionAlgorithm))
        if (digestEncryptionParameters == null) {
        ...
        try {
            ...
            return AlgorithmIdentifier(
                digestEncryptionAlgOID,
                ASN1Primitive.fromByteArray(params.getEncoded())
            )
        } catch (ioe: IOException) {
