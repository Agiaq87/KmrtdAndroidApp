...
class CardSecurityFile : Serializable {
    ...
    constructor(
        ...
        this.encryptedDigest = SignedDataUtil.signData(
            digestAlgorithm,
            digestEncryptionAlgorithm,
            CONTENT_TYPE_OID,
            contentInfo,
    ...
    protected fun readContent(inputStream: InputStream?) {
        val signedData = SignedDataUtil.readSignedData(inputStream)
        ...
        val certificates = SignedDataUtil.getCertificates(signedData)
        this.certificate =
            if (certificates == null || certificates.isEmpty()) null else certificates.get(
                certificates.size - 1
            )
    ...
    protected fun writeContent(outputStream: OutputStream) {
        try {
            val contentInfo: ContentInfo =
                Companion.toContentInfo(CONTENT_TYPE_OID, securityInfos!!)
    ...
    companion object {
        ...
        private fun getSecurityInfos(signedData: SignedData): MutableSet<SecurityInfo> {
            val encapsulatedContent = SignedDataUtil.getContent(signedData)
