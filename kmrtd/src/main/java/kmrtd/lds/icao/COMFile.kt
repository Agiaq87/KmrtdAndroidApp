...
class COMFile : AbstractTaggedLDSFile {
    ...
    override fun readContent(`in`: InputStream?) {
        ...
        require(versionLDSTag == VERSION_LDS_TAG) {
            "Excepected VERSION_LDS_TAG (" + Integer.toHexString(
                VERSION_LDS_TAG
            ) + "), found " + Integer.toHexString(versionLDSTag)
        }
        ...
        require(tagListTag == TAG_LIST_TAG) {
            "Expected TAG_LIST_TAG (" + Integer.toHexString(
                TAG_LIST_TAG
            ) + "), found " + Integer.toHexString(tagListTag)
        }
        /* int tagListLength = */
        tlvIn.readLength()
