...
class SODFile : AbstractTaggedLDSFile {
    ...
    ) : super(LDSFile.Companion.EF_SOD_TAG) {
        try {
            val contentInfo: ContentInfo = toContentInfo(
                ICAO_LDS_SOD_OID,
                digestAlgorithm,
                dataGroupHashes,
                ldsVersion,
            ...
            val encryptedDigest = SignedDataUtil.signData(
                digestAlgorithm,
     ...
     */
    constructor(inputStream: InputStream?) : super(LDSFile.Companion.EF_SOD_TAG, inputStream) {
    ...
    override fun hashCode(): Int {
        return 11 * encoded.contentHashCode() + 111
    }
    ...
    companion object {
        ...
        private fun getDigestAlgorithm(ldsSecurityObject: LDSSecurityObject): String? {
            try {
                return SignedDataUtil.lookupMnemonicByOID(
                    ldsSecurityObject.getDigestAlgorithmIdentifier().getAlgorithm().getId()
                )
        ...
        private fun toContentInfo(
            ...
            val digestAlgorithmIdentifier = AlgorithmIdentifier(
