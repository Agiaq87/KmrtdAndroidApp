...
object Util {
    ...
    fun deriveKey(
        ...
        return deriveKey(
            ...
            nonce,
            mode,
            PassportService.Companion.NO_PACE_KEY_REFERENCE
        )
    }
    ...
    fun computeKeySeed(
        ...
        val text = StringBuilder()
            ...
            .append(MRZInfo.Companion.checkDigit(documentNumber))
            .append(dateOfBirth)
            .append(MRZInfo.Companion.checkDigit(dateOfBirth))
            .append(dateOfExpiry)
            .append(MRZInfo.Companion.checkDigit(dateOfExpiry))
            .toString()
    ...
    fun inferDigestAlgorithmFromSignatureAlgorithm(signatureAlgorithm: String): String? {
        ...
        if (signatureAlgorithmToUppercase.contains("WITH")) {
            val components: Array<String?> =
                signatureAlgorithmToUppercase.split("WITH".toRegex()).dropLastWhile { it.isEmpty() }
                    .toTypedArray()
     ...
     */
    fun inferDigestAlgorithmFromCipherAlgorithmForKeyDerivation(
        cipherAlg: String,
        keyLength: Int
    ): String {
    ...
    fun toExplicitDHParameterSpec(params: DHParameters): DHParameterSpec {
        ...
        if (q == null) {
            ...
            return DHCParameterSpec(p, generator, q)
        }
    }
    ...
    fun getApproximateSignatureSize(key: Key?): Int {
        if (key is RSAPublicKey) {
            ...
            val keySize =
                ceil(key.getParams().getCurve().getField().getFieldSize().toDouble()).toInt()
            return 2 * keySize
   ...
   */
        ...
        try {
            ...
            if ("EC" == algorithm || "ECDH" == algorithm || (publicKey is ECPublicKey)) {
                ...
                try {
                    ...
                    check(SecurityInfo.Companion.ID_EC_PUBLIC_KEY == algOID) { "Was expecting id-ecPublicKey (" + SecurityInfo.Companion.ID_EC_PUBLIC_KEY_TYPE + "), found " + algOID }
                ...
                try {
                    val subjectPublicKeyInfo =
                        SubjectPublicKeyInfo.getInstance((asn1In.readObject()))
                    val algorithmIdentifier = subjectPublicKeyInfo!!.getAlgorithm()
    ...
    fun reconstructPublicKey(publicKey: PublicKey?): PublicKey? {
        ...
        try {
            ...
            return KeyFactory.getInstance("EC", bouncyCastleProvider)
                .generatePublic(explicitPublicKeySpec)
        } catch (e: Exception) {
     ...
     */
    fun addMissingParametersToPublicKey(
    ...
    fun inferProtocolIdentifier(publicKey: PublicKey): String? {
        ...
        if ("EC" == algorithm || "ECDH" == algorithm) {
            return SecurityInfo.Companion.ID_PK_ECDH
        } else if ("DH" == algorithm) {
            return SecurityInfo.Companion.ID_PK_DH
        } else {
    ...
    fun getCipher(algorithm: String): Cipher {
        try {
            ...
            LOGGER.log(
                Level.FINE,
                "Default provider could not provide this cipher, falling back to explicit BC",
                e
            )
    ...
    fun getMac(algorithm: String): Mac {
        try {
            ...
            LOGGER.log(
                Level.FINE,
                "Default provider could not provide this Mac, falling back to explicit BC",
                e
            )
    ...
    fun getSignature(algorithm: String): Signature? {
        try {
            ...
            LOGGER.log(
                Level.FINE,
                "Default provider could not provide this Signature, falling back to explicit BC",
                e
            )
    ...
    fun toOIDBytes(oid: String): ByteArray? {
        ...
        try {
            val oidTLVIn =
                TLVInputStream(ByteArrayInputStream(ASN1ObjectIdentifier(oid).getEncoded()))
            try {
   ...
   */
    ...
    private const val RFC5114_2048_224_Q =
    ...
    @Deprecated(
        "Existence of a " hidden SNFS
        " backdoor cannot be ruled out. see https://eprint.iacr.org/2016/961.pdf"
    )
    val RFC5114_1024_160: DHParameters =
        fromPGQ(RFC5114_1024_160_P, RFC5114_1024_160_G, RFC5114_1024_160_Q)

    @Deprecated(
        "Existence of a " hidden SNFS
        " backdoor cannot be ruled out. see https://eprint.iacr.org/2016/961.pdf"
    )
    val RFC5114_2048_224: DHParameters =
        fromPGQ(RFC5114_2048_224_P, RFC5114_2048_224_G, RFC5114_2048_224_Q)

    @Deprecated(
        "Existence of a " hidden SNFS
        " backdoor cannot be ruled out. see https://eprint.iacr.org/2016/961.pdf"
    )
    val RFC5114_2048_256: DHParameters =
        fromPGQ(RFC5114_2048_256_P, RFC5114_2048_256_G, RFC5114_2048_256_Q)
