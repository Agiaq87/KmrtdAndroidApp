...
class PACEProtocol(
    ...
    fun doPACE(
        ...
        try {
            return doPACE(
                accessKey,
                deriveStaticPACEKey(accessKey, oid),
                oid,
                staticParameters,
                parameterId
            )
    ...
    private fun doPACE(
        ...
        val mappingType: PACEInfo.MappingType =
            PACEInfo.Companion.toMappingType(oid) /* Either GM, CAM, or IM. */
        val agreementAlg: String =
            PACEInfo.Companion.toKeyAgreementAlgorithm(oid) /* Either DH or ECDH. */
        val cipherAlg: String =
            PACEInfo.Companion.toCipherAlgorithm(oid) /* Either DESede or AES. */
        val digestAlg: String =
            PACEInfo.Companion.toDigestAlgorithm(oid) /* Either SHA-1 or SHA-256. */
        val keyLength: Int =
            PACEInfo.Companion.toKeyLength(oid) /* Of the enc cipher. Either 128, 192, or 256. */
        ...
        try {
            ...
            var paceKeyReference: Byte = PassportService.Companion.MRZ_PACE_KEY_REFERENCE
       ...
       */
            val referencePrivateKeyOrForComputingSessionKey =
                if (parameterId == null) null else Util.i2os(parameterId)

            /* Send to the PICC. */
            service.sendMSESetATMutualAuth(
                wrapper!!,
                oid,
                paceKeyReference.toInt(),
                referencePrivateKeyOrForComputingSessionKey!!
            )
        } catch (cse: CardServiceException) {
            throw CardServiceProtocolException(
                "PICC side error in static PACE key derivation step",
                0,
                cse
            )
        } catch (e: Exception) {
            ...
            throw CardServiceProtocolException(
                "PCD side error in static PACE key derivation step",
                0,
                e
            )
        }
     ...
     */
        val mappingResult =
            doPACEStep2(mappingType, agreementAlg, staticParameters, piccNonce, staticPACECipher)
     ...
     */
        val ephemeralPICCPublicKey =
            doPACEStep3ExchangePublicKeys(ephemeralPCDKeyPair.getPublic(), ephemeralParams)
        ...
        try {
            ...
            throw CardServiceProtocolException(
                "Security exception during secure messaging key derivation",
                3,
                gse
            )
        }
     ...
     */
        val encryptedChipAuthenticationData =
            doPACEStep4(oid, mappingType, ephemeralPCDKeyPair, ephemeralPICCPublicKey, macKey)
     ...
     */
        try {
            val ssc = if (wrapper == null) 0L else wrapper!!.getSendSequenceCounter()
            if (cipherAlg.startsWith("DESede")) {
                ...
                wrapper = AESSecureMessagingWrapper(
                    encKey,
                    macKey,
                    maxTranceiveLengthForSecureMessaging,
                    shouldCheckMAC,
            ...
            throw CardServiceProtocolException(
                "Security exception in secure messaging establishment",
                4,
                gse
            )
        }

        if (PACEInfo.MappingType.CAM == mappingType) {
            ...
            try {
                ...
                decryptCipher.init(
                    Cipher.DECRYPT_MODE, encKey, IvParameterSpec(
                        IV_FOR_PACE_CAM_DECRYPTION
                    )
                )
            ...
            return PACECAMResult(
                accessKey, agreementAlg, cipherAlg, digestAlg, keyLength,
        ...
        /* GM or IM result. */
        return PACEResult(
            accessKey, mappingType, agreementAlg, cipherAlg, digestAlg, keyLength,
   ...
   */
    ...
    fun doPACEStep1(staticPACEKey: SecretKey?, staticPACECipher: Cipher): ByteArray {
        ...
        try {
            ...
            throw CardServiceProtocolException(
                "PCD side exception in tranceiving nonce step",
                1,
                gse
            )
        } catch (cse: CardServiceException) {
            throw CardServiceProtocolException(
                "PICC side exception in tranceiving nonce step",
                1,
                cse
            )
        }
   ...
   */
    ...
    @Throws(CardServiceProtocolException::class)
    fun doPACEStep2(
        mappingType: PACEInfo.MappingType,
        agreementAlg: String,
        ...
        when (mappingType) {
            PACEInfo.MappingType.CAM, PACEInfo.MappingType.GM -> return doPACEStep2GM(
                agreementAlg,
                params,
                piccNonce
            )

            PACEInfo.MappingType.IM -> return doPACEStep2IM(
                agreementAlg,
                params,
                piccNonce,
                staticPACECipher
    ...
    fun doPACEStep2GM(
        ...
        try {
            val keyPairGenerator: KeyPairGenerator =
                KeyPairGenerator.getInstance(agreementAlg, BC_PROVIDER)
            ...
            val pcdMappingEncodedPublicKey: ByteArray =
                encodePublicKeyForSmartCard(pcdMappingPublicKey)
            val step2Data = TLVUtil.wrapDO(0x81, pcdMappingEncodedPublicKey)

            val step2Response = service.sendGeneralAuthenticate(
                wrapper!!,
                step2Data,
                if (step2Data.size > 233) PassportService.Companion.EXTENDED_MAX_TRANCEIVE_LENGTH else maxTranceiveLengthForProtocol,
                false
            )

            val piccMappingEncodedPublicKey = TLVUtil.unwrapDO(0x82, step2Response)
            val piccMappingPublicKey: PublicKey =
                decodePublicKeyFromSmartCard(piccMappingEncodedPublicKey, params)
            ...
            if ("ECDH" == agreementAlg) {
                ...
                val ephemeralParameters: AlgorithmParameterSpec = mapNonceGMWithECDH(
                    piccNonce,
                    mappingSharedSecretPoint,
                    params as ECParameterSpec
                )
                return PACEGMWithECDHMappingResult(
                    params,
                    piccNonce,
                ...
                val mappingAgreement = KeyAgreement.getInstance(agreementAlg)
                mappingAgreement.init(pcdMappingPrivateKey)
                mappingAgreement.doPhase(piccMappingPublicKey, true)
                val mappingSharedSecretBytes = mappingAgreement.generateSecret()
                val ephemeralParameters: AlgorithmParameterSpec = mapNonceGMWithDH(
                    piccNonce,
                    Util.os2i(mappingSharedSecretBytes),
                    params as DHParameterSpec
                )
                return PACEGMWithDHMappingResult(
                    params,
                    piccNonce,
                    piccMappingPublicKey,
                    pcdMappingKeyPair,
                    mappingSharedSecretBytes,
                    ephemeralParameters
                )
            } else {
   ...
   */
    ...
    fun doPACEStep2IM(
       ...
       */
            /* byte[] step2Response = */
            service.sendGeneralAuthenticate(
                wrapper!!,
                step2Data,
                if (step2Data.size > 233) PassportService.Companion.EXTENDED_MAX_TRANCEIVE_LENGTH else maxTranceiveLengthForProtocol,
                false
            ...
            if ("ECDH" == agreementAlg) {
                val ephemeralParameters: AlgorithmParameterSpec = mapNonceIMWithECDH(
                    piccNonce,
                    pcdNonce,
     ...
     */
    @Throws(CardServiceProtocolException::class)
    fun doPACEStep3GenerateKeyPair(
        agreementAlg: String,
        ephemeralParams: AlgorithmParameterSpec?
    ): KeyPair {
        try {
            val keyPairGenerator: KeyPairGenerator =
                KeyPairGenerator.getInstance(agreementAlg, BC_PROVIDER)
            ...
            throw CardServiceProtocolException(
                "PCD side error during generation of PCD key pair",
                3,
                gse
            )
        }
   ...
   */
     ...
     */
    @Throws(CardServiceProtocolException::class)
    fun doPACEStep3ExchangePublicKeys(
        pcdPublicKey: PublicKey,
        ephemeralParams: AlgorithmParameterSpec
    ): PublicKey {
        try {
            ...
            val step3Response = service.sendGeneralAuthenticate(
                wrapper!!,
                step3Data,
                if (step3Data.size > 233) PassportService.Companion.EXTENDED_MAX_TRANCEIVE_LENGTH else maxTranceiveLengthForProtocol,
                false
            )
            val piccEncodedPublicKey = TLVUtil.unwrapDO(0x84, step3Response)
            val piccPublicKey: PublicKey =
                decodePublicKeyFromSmartCard(piccEncodedPublicKey, ephemeralParams)

            if (keysAreEqual(pcdPublicKey, piccPublicKey)) {
                throw CardServiceProtocolException(
                    "PCD's public key and PICC's public key are the same in key agreement step!",
                    3
                )
            }
     ...
     */
    @Throws(CardServiceProtocolException::class)
    fun doPACEStep3KeyAgreement(
        agreementAlg: String,
        pcdPrivateKey: PrivateKey,
   ...
   */
    ...
    fun doPACEStep4(
        ...
        try {
            val pcdToken: ByteArray = generateAuthenticationToken(oid, macKey, piccPublicKey)
            ...
            try {
                ...
                val expectedPICCToken: ByteArray =
                    generateAuthenticationToken(oid, macKey, pcdKeyPair.getPublic())
            ...
            throw CardServiceProtocolException(
                "PCD side exception in authentication token generation step",
                4,
                e
            )
        }
    ...
    private fun checkConsistency(
        ...
        requireNotNull(agreementAlg) { "Unknown agreement algorithm" }

        /* Agreement algorithm should be ECDH or DH. */
        require(
            "ECDH".equals(agreementAlg, ignoreCase = true) || "DH".equals(
                agreementAlg,
                ignoreCase = true
            )
        ...
        require(
            "DESede".equals(cipherAlg, ignoreCase = true) || "AES".equals(
                cipherAlg,
                ignoreCase = true
            )
        ...
        require(
            "SHA-1".equals(digestAlg, ignoreCase = true) || "SHA1".equals(
                digestAlg,
                ignoreCase = true
            )
                    || "SHA-256".equals(digestAlg, ignoreCase = true) || "SHA256".equals(
                digestAlg,
                ignoreCase = true
            )
        ...
        require(
            !("ECDH".equals(
                agreementAlg,
                ignoreCase = true
            ) && params !is ECParameterSpec)
        ) { "Expected ECParameterSpec for agreement algorithm \"" + agreementAlg + "\", found " + params.javaClass.getCanonicalName() }
        require(
            !("DH".equals(
                agreementAlg,
                ignoreCase = true
            ) && params !is DHParameterSpec)
        ) { "Expected DHParameterSpec for agreement algorithm \"" + agreementAlg + "\", found " + params.javaClass.getCanonicalName() }
    }
    ...
    companion object {
        ...
        /** Constant used in IM pseudo random number mapping, see Doc 9303 - Part 11, 4.4.3.3.2.  */ /* a668892a7c41e3ca739f40b057d85904, 16 bytes, 128 bits  */
        ...
        /** Constant used in IM pseudo random number mapping, see Doc 9303 - Part 11, 4.4.3.3.2.  */ /* a4e136ac725f738b01c1f60217c188ad, 16 bytes, 128 bits */
        ...
        /** Constant used in IM pseudo random number mapping, see Doc 9303 - Part 11, 4.4.3.3.2.  */ /* d463d65234124ef7897054986dca0a174e28df758cbaa03f240616414d5a1676, 32 bytes, 256 bits */
        ...
        /** Constant used in IM pseudo random number mapping, see Doc 9303 - Part 11, 4.4.3.3.2.  */ /* 54bd7255f0aaf831bec3423fcf39d69b6cbf066677d0faae5aadd99df8e53517, 32 bytes, 256 bits */
        ...
        fun computeKeySeedForPACE(accessKey: AccessKeySpec): ByteArray? {
            ...
            if (accessKey is BACKeySpec) {
                ...
                var documentNumber = bacKey.documentNumber
                val dateOfBirth = bacKey.dateOfBirth
                val dateOfExpiry = bacKey.dateOfExpiry
     ...
     */
        ...
        fun mapNonceGMWithDH(
            ...
            val mappedGenerator =
                generator.modPow(Util.os2i(nonceS), p).multiply(sharedSecretH).mod(p)
         ...
         * @param cipherAlgorithm the cipher algorithm to be used by the pseudo random function (either `"AES"` or `"DESede"`)
         * @param params the static domain parameters
         * 
        ...
        fun mapNonceIMWithECDH(
            ...
            val mappedGenerator: ECPoint = icartPointEncode(t, params)
            return ECParameterSpec(
                EllipticCurve(ECFieldFp(p), a, b),
                mappedGenerator,
                order,
   ...
   */
         ...
         * @param algorithm the algorithm for block cipher E (either `"AES"` or `"DESede"`)
         * 
         ...
         */
        @Throws(GeneralSecurityException::class)
        fun pseudoRandomFunction(
            s: ByteArray,
            t: ByteArray,
            p: BigInteger,
            algorithm: String
        ): ByteArray {
            require(!(s == null || t == null)) { "Null nonce" }

            val l = s.size * 8
            val k = t.size * 8 /* Key size in bits. */

            var c0: ByteArray? = null
            var c1: ByteArray? = null
            when (l) {
                128 -> {
                    c0 = C0_LENGTH_128
                    c1 = C1_LENGTH_128
            ...
            val cipher =
                Cipher.getInstance(algorithm + (if (algorithm.endsWith("/CBC/NoPadding")) "" else "/CBC/NoPadding"))
            val blockSize = cipher.getBlockSize() /* in bytes */

            val zeroIV = IvParameterSpec(ByteArray(blockSize))

            cipher.init(Cipher.ENCRYPT_MODE, SecretKeySpec(t, algorithm), zeroIV)
            var key = cipher.doFinal(s)
            ...
            try {
                ...
                while (n * l < p.bitLength() + 64) {
                    cipher.init(
                        Cipher.ENCRYPT_MODE,
                        SecretKeySpec(key, 0, k / 8, algorithm),
                        zeroIV
                    )
   ...
   */
        ...
        fun icartPointEncode(t: BigInteger, params: ECParameterSpec?): ECPoint {
            ...
            val x2 = b.negate().multiply(onePlusAlphaPlusAlphaSq)
                .multiply(a.multiply(alphaPlusAlphaSq).modPow(pMinus2, p)).mod(p)
            ...
            /* 7. */
            val pPlusOneOverFour =
                p.add(BigInteger.ONE).multiply(BigInteger.valueOf(4).modInverse(p)).mod(p)
            val pMinusOneMinusPPlusOneOverFour =
                p.subtract(BigInteger.ONE).subtract(pPlusOneOverFour)
            val aa = h2.modPow(pMinusOneMinusPPlusOneOverFour, p)

            val aaSqTimesH2 = aa.modPow(BigInteger.valueOf(2), p).multiply(h2).mod(p)

            val xy =
                if (aaSqTimesH2 == BigInteger.ONE) ECPoint(x2, aa.multiply(h2).mod(p)) else ECPoint(
                    x3,
                    aa.multiply(u).mod(p)
                )
        ...
        fun updateParameterSpec(publicKey: PublicKey, privateKey: PrivateKey): PublicKey? {
            ...
            if ("EC" == publicKeyAlgorithm || "ECDH" == publicKeyAlgorithm) {
                ...
                val keySpec: KeySpec = ECPublicKeySpec(
                    (publicKey as ECPublicKey).getW(),
                    (privateKey as ECPrivateKey).getParams()
                )
                ...
                val keySpec: KeySpec = DHPublicKeySpec(
                    dhPublicKey.getY(),
                    privateKeyParams.getP(),
                    privateKeyParams.getG()
                )
                return keyFactory.generatePublic(keySpec)
            } else {
                throw NoSuchAlgorithmException("Unsupported key type public: " + publicKeyAlgorithm + ", private: " + privateKeyAlgorithm)
        ...
        fun generateAuthenticationToken(
            ...
            val cipherAlg: String = PACEInfo.Companion.toCipherAlgorithm(oid)
            val macAlg: String = inferMACAlgorithmFromCipherAlgorithm(cipherAlg)
        ...
        fun encodePublicKeyDataObject(
            oid: String,
            publicKey: PublicKey,
            isContextKnown: Boolean = true
        ): ByteArray {
            val byteArrayOutputStream = ByteArrayOutputStream()
            val tlvOutputStream = TLVOutputStream(byteArrayOutputStream)
            try {
        ...
        fun encodeECPointForSmartCard(w: ECPoint, bitLength: Int): ByteArray {
            try {
                val bOut = ByteArrayOutputStream()
                bOut.write(Util.ecPoint2OS(w, bitLength))
                val encodedPublicKey = bOut.toByteArray()
                bOut.close()
         ...
         */
        fun decodePublicKeyFromSmartCard(
            ...
            try {
                if (params is ECParameterSpec) {
                    ...
                    return Util.getPublicKey(
                        "DH",
                        DHPublicKeySpec(y, dhParams.getP(), dhParams.getG())
                    )
                }
         ...
         */
        @Throws(GeneralSecurityException::class)
        private fun generateAuthenticationToken(
            oid: String,
            mac: Mac,
            publicKey: PublicKey
        ): ByteArray {
        ...
        private fun fixDocumentNumber(documentNumber: String): String {
            ...
            val minDocumentNumber =
                documentNumber.replace('<', ' ').trim { it <= ' ' }.replace(' ', '<')

            /* The document number, including trailing '<' until length 9. */
            val result = StringBuilder(minDocumentNumber)
         ...
         * be thrown doing so. Returns `false` if an exception is thrown.
         * 
        ...
        private fun inferMACAlgorithmFromCipherAlgorithm(cipherAlg: String): String {
            requireNotNull(cipherAlg) { "Cannot infer MAC algorithm from cipher algorithm null" }
