...
class PACEProtocol(
    ...
    private fun doPACE(
        ...
        val digestAlg = PACEInfo.toDigestAlgorithm(oid) /* Either SHA-1 or SHA-256. */
        ...
        val ephemeralParams = mappingResult.ephemeralParameters
        ...
        val ephemeralPICCPublicKey =
            doPACEStep3ExchangePublicKeys(ephemeralPCDKeyPair.public, ephemeralParams)
    ...
    fun doPACEStep2GM(
        ...
        try {
            ...
            val step2Response = service.sendGeneralAuthenticate(
                wrapper,
                step2Data,
                if (step2Data.size > 233) PassportService.EXTENDED_MAX_TRANCEIVE_LENGTH else maxTranceiveLengthForProtocol,
                false
            )
    ...
    fun doPACEStep3ExchangePublicKeys(
        ...
        try {
            ...
            val step3Response = service.sendGeneralAuthenticate(
                wrapper,
                step3Data,
                if (step3Data.size > 233) PassportService.EXTENDED_MAX_TRANCEIVE_LENGTH else maxTranceiveLengthForProtocol,
                false
            )
        ...
        try {
            ...
            try {
                ...
                val expectedPICCToken: ByteArray =
                    generateAuthenticationToken(oid, macKey, pcdKeyPair.public)
    ...
    companion object {
        ...
        fun deriveStaticPACEKey(accessKey: AccessKeySpec, oid: String?): SecretKey {
            val cipherAlg = PACEInfo.toCipherAlgorithm(oid) /* Either DESede or AES. */
            val keyLength =
                PACEInfo.toKeyLength(oid) /* Of the enc cipher. Either 128, 192, or 256. */
        ...
        fun computeKeySeedForPACE(accessKey: AccessKeySpec): ByteArray {
            ...
            if (accessKey is BACKeySpec) {
                var documentNumber = accessKey.getDocumentNumber()
                val dateOfBirth = accessKey.getDateOfBirth()
                val dateOfExpiry = accessKey.getDateOfExpiry()
     ...
     */
        ...
        fun mapNonceIMWithDH(
            ...
            val q =
                if (params is PACEInfo.DHCParameterSpec) params.q else BigInteger.ONE // FIXME: What if q not available? We use 1 here? Should be p-1? -- MO
        ...
        fun updateParameterSpec(publicKey: PublicKey, privateKey: PrivateKey): PublicKey? {
            ...
            if ("EC" == publicKeyAlgorithm || "ECDH" == publicKeyAlgorithm) {
                ...
                val keySpec: KeySpec = ECPublicKeySpec(
                    ...
                    (privateKey as ECPrivateKey).params
                )
            ...
            try {
                if (params is ECParameterSpec) {
                    ...
                    return Util.getPublicKey(
                        "DH",
                        DHPublicKeySpec(y, params.getP(), params.getG())
                    )
                }
     ...
     */
         ...
         */
            return if (cipherAlg.startsWith("DESede")) {
