 ...
 * @since 0.5.6
 */
class EACTAProtocol(service: APDULevelEACTACapable, wrapper: SecureMessagingWrapper?) {
    private val service: APDULevelEACTACapable
   ...
   */
    ...
    fun doTA(
        ...
        var caReference: CVCPrincipal? = caReference
        try {
            ...
            val caKeyHash = chipAuthenticationResult.getKeyHash()
       ...
       */
            ...
            val firstCertRole: Role = firstCert.authorizationTemplate.role
            if (Role.CVCA.equals(firstCertRole)) {
                val firstCertHolderReference: CVCPrincipal = firstCert.holderReference
            ...
            val firstCertAuthorityReference: CVCPrincipal = firstCert.authorityReference
            ...
            val lastCertRole: Role = lastCert.authorizationTemplate.role
            ...
            val terminalCert = lastCert

            /* Have the MRTD check our chain. */
            for (cert in terminalCertificates) {
                try {
                    val authorityReference: CVCPrincipal = cert.authorityReference
           ...
           */
                    val authorityRefBytes: ByteArray? = TLVUtil.wrapDO(
                ...
                try {
                    ...
                    val tlvSigOut: TLVOutputStream = TLVOutputStream(sigOut)
            ...
            /* Step 3: MSE Set AT */
            try {
                val holderRef: CVCPrincipal = terminalCert.holderReference
                val holderRefBytes: ByteArray? = TLVUtil.wrapDO(
                    0x83, holderRef.getName().toByteArray(
                        charset("ISO-8859-1")
                    )
                )
                /*
         ...
         */
            ...
            try {
                ...
                checkNotNull(sigAlg) { "Could not determine signature algorithm for terminal certificate " + terminalCert.holderReference.getName() }
    ...
    companion object {
        ...
        fun deriveIdentifier(documentNumber: String?): ByteArray {
            ...
            try {
                ...
                idPICC[documentNumberLength] =
                    MRZInfo.Companion.checkDigit(documentNumber).code.toByte()
                return idPICC
