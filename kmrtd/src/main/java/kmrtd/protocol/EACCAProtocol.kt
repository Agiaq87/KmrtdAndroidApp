 ...
 */(
     ...
     */
    var wrapper: SecureMessagingWrapper?,
    ...
    fun doCA(
        ...
        try {
            agreementAlg = ChipAuthenticationInfo.Companion.toKeyAgreementAlgorithm(oid)
        ...
        try {
            ...
            val sharedSecret: ByteArray? =
                computeSharedSecret(agreementAlg, piccPublicKey, pcdPrivateKey)
    ...
    companion object {
        ...
        @Throws(CardServiceException::class)
        fun sendPublicKey(
            service: APDULevelEACCACapable,
            wrapper: SecureMessagingWrapper?,
            oid: String?,
            keyId: BigInteger?,
            pcdPublicKey: PublicKey?
        ) {
            val agreementAlg: String = ChipAuthenticationInfo.Companion.toKeyAgreementAlgorithm(oid)
            val cipherAlg: String = ChipAuthenticationInfo.Companion.toCipherAlgorithm(oid)
            val keyData: ByteArray = getKeyData(agreementAlg, pcdPublicKey)

            if (cipherAlg.startsWith("DESede")) {
                var idData: ByteArray? = null
                if (keyId != null) {
                    val keyIdBytes = Util.i2os(keyId)
                    idData = TLVUtil.wrapDO(0x84, keyIdBytes) /* FIXME: Constant for 0x84. */
                }
                try {
                    service.sendMSEKAT(
                        wrapper,
                        TLVUtil.wrapDO(0x91, keyData),
                        idData
                    ) /* FIXME: Constant for 0x91. */
                } catch (e: Exception) {
                ...
                try {
                    val data = TLVUtil.wrapDO(0x80, keyData) /* FIXME: Constant for 0x80. */
                    sendGeneralAuthenticate(service, wrapper, data)
                } catch (e: Exception) {
                    throw CardServiceProtocolException(
                        "Exception during General Authenticate",
                        2,
                        e
                    )
                }
        ...
        fun restartSecureMessaging(
            ...
            val cipherAlg: String = ChipAuthenticationInfo.Companion.toCipherAlgorithm(oid)
            val keyLength: Int = ChipAuthenticationInfo.Companion.toKeyLength(oid)
            ...
            if (cipherAlg.startsWith("DESede")) {
                return DESedeSecureMessagingWrapper(
                    ksEnc,
                    ksMac,
                    maxTranceiveLength,
                    shouldCheckMAC,
                ...
                return AESSecureMessagingWrapper(
                    ksEnc,
                    ksMac,
                    maxTranceiveLength,
                    shouldCheckMAC,
        ...
        fun getKeyHash(agreementAlg: String?, pcdPublicKey: PublicKey?): ByteArray? {
            if ("DH" == agreementAlg || pcdPublicKey is DHPublicKey) {
                ...
                val t = Util.i2os(pcdECPublicKey.getQ().getAffineXCoord().toBigInteger())
                val keySize =
                    ceil(pcdECPublicKey.getParameters().getCurve().getFieldSize() / 8.0).toInt()
         ...
         */
        @Throws(CardServiceException::class)
        private fun sendGeneralAuthenticate(
            service: APDULevelEACCACapable,
            wrapper: SecureMessagingWrapper?,
            data: ByteArray?
        ) {
            try {
                ...
                LOGGER.log(
                    Level.WARNING,
                    "Failed to send GENERAL AUTHENTICATE, falling back to command chaining",
                    cse
                )
                val segments = Util.partition(COMMAND_CHAINING_CHUNK_SIZE, data)
        ...
        private fun inferChipAuthenticationOIDfromPublicKeyOID(publicKeyOID: String?): String? {
            if (SecurityInfo.Companion.ID_PK_ECDH == publicKeyOID) {
                /*
       ...
       */
                ...
                return SecurityInfo.Companion.ID_CA_ECDH_3DES_CBC_CBC
            } else if (SecurityInfo.Companion.ID_PK_DH == publicKeyOID) {
                /*
       ...
       */
                ...
                return SecurityInfo.Companion.ID_CA_DH_3DES_CBC_CBC
            } else {
